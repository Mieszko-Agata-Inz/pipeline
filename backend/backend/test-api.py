...
model_name_cold = "xgb1"  # Replace with a model name that exists in coldstart_models
model_name_hot = "lstm1"  # Replace with a model name that exists in hot_models
...
@pytest.fixture
def mock_models():
    with patch('backend.main.coldstart_models', new_callable=lambda: {"xgb1": (None, Mock()), "xgb2": (None, Mock()), "xgb3": (None, Mock())}), \
         patch('backend.main.hot_models', new_callable=lambda: {"lstm1": (None, Mock()), "lstm2": (None, Mock()), "lstm3": (None, Mock())}):
        yield
...
def test_no_data(mock_redis):
    ...
    result = get_forecast("some_geohash", {}, {})
...
def test_cold_model_prediction(mock_redis):
    ...
    mock_redis.ft().search.side_effect = [Mock(docs=[]), Mock(docs=[Mock(json=json.dumps({"humidity": 50, "temp": 20, "timestamp": datetime.datetime.timestamp(datetime.datetime.utcnow())}))])] 
    # Calling the function with mocked dependencies
    result = get_forecast("some_geohash", {"xgb1": [-1, dummy_xgb()], "xgb2": [-1, dummy_xgb()], "xgb3": [-1, dummy_xgb()]}, {})
...
def test_get_raw_data(mock_redis2):
    ...
    timestamp = datetime.datetime.timestamp(current_time) - 30  # 30 seconds before current time
